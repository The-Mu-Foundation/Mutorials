...
module.exports = (app, mongo) => {
    app.all(/^(\/reviewer).*$/, (req, res, next) => {
        if (req.isAuthenticated() && (adminList.includes(req.user.username) || req.user.reviewer == true)) {
            next()
        } else if (req.isAuthenticated() && !(adminList.includes(req.user.username)) && (!req.user.reviewer || req.user.reviewer == false)) {
