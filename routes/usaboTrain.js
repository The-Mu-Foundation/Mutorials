...
module.exports = (app, mongo) => {
    ...
    app.get('/train/usabo/chooseCategories', (req, res) => {
        const qNum = 1;
        if (req.user.rating[req.params.round] == -1) { //check to see if redir needed
            res.redirect('/train/' + 'usabo' + '/proficiency'); //ROUTING FIX
        }
        else {
            res.render(VIEWS + 'usabo/train/chooseCategories.ejs', { round: req.params.round, categories: CATEGORIES, qNum: qNum, unitPresets: presetUnitOptions[req.params.round], pageName: "Train Categories" });
        }
    });
    ...
    app.get('/train/usabo/displayQuestion', async (req, res) => {
        ...
        if (q && units.some(r => q.units.includes(r))) {
            ...
            getQuestions(mongo.USABOQues, floor, ceiling, req.params.round, categories).then(qs => {
                ...
                console.log(curQ);
                if (!curQ) {
                    req.flash('errorFlash', 'We couldn\'t find any questions for your rating in the categories you selected.');
                    res.redirect('/train/' + 'usabo' + '/chooseCategories');
                    return;
                }
